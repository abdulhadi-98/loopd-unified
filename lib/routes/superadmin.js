...
const HARDCODED_ADMIN = {
  email:    process.env.SUPERADMIN_EMAIL    || 'admin@loopd.app',
  password: process.env.SUPERADMIN_PASSWORD || 'Loopd@Admin123',
  name:     'Super Admin',
  id:       'hardcoded-superadmin',
};
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    // Hardcoded fallback — works even if DB is empty
    if (email === HARDCODED_ADMIN.email && password === HARDCODED_ADMIN.password) {
      const token = jwt.sign(
        { superadminId: HARDCODED_ADMIN.id, role: 'superadmin' },
        process.env.JWT_SECRET,
        { expiresIn: '7d' }
      );
      return res.json({ token, admin: { id: HARDCODED_ADMIN.id, name: HARDCODED_ADMIN.name, email: HARDCODED_ADMIN.email } });
    }
